...
exports.addQuestion = async function (req, res) {
  let userInfo = req.body;
  await Question.create(userInfo)
  res.json({
    status: 0,
    msg: 'sucess'
  })
...
exports.getQuestionOne = async function (req, res) {
  let { id } = req.body;
  const r = await Question.findOne({
    _id: id
  })
  res.json({
    status: 0,
...
exports.deletePaper = async function (req, res) {
  let { id } = req.body;
  const r = await Paper.deleteOne({
    _id: id
  })
  res.json({
    status: 0,
...
exports.getOnePaper = async function (req, res) {
  let { id } = req.body;
  const r = await Paper.findOne({
    _id: id
  })
  res.json({
    status: 0,
...
exports.signup = async function (req, res) {
  ...
  const r = await Teacher.findOne(userInfo)
  if (r) {
    res.json({
      status: 0,
      msg: 'sucess',
...
exports.getUserInfo = async function (req, res) {
  let id = req.body.id;
  const r = await Teacher.findOne({ _id: id })
  res.json({
    status: '0',
    msg: '',
    data: r
